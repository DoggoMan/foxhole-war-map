...
function convertCoords(regionId, x, y) {
  ...
  if (!region) throw new Error(`Cannot find region ${regionId}`)
      ...
      .then((data) => {
        ...
        data.forEach((region) => {
          ...
          region.mapTextItems.map((mapTextItem) => {
            ...
            let mapTextItemObject = new MapTextItem(
              region.regionId,
              mapTextItem.text,
              mapTextItem.mapMarkerType,
              coords.xcoord,
              coords.ycoord
            )
      ...
      .then((data) => {
        ...
        data.forEach((region) => {
          ...
          region.mapItems.map((mapItem) => {
            ...
            const mapItemObject = new MapItem(
              region.regionId,
              mapItem.teamId,
              mapItem.iconType,
              coords.xcoord,
        ...
        .then((dynamicData) => {
          ...
          dynamicData.map((mapItem) => {
            try {
              ...
              let marker = L.marker([mapItem.y, mapItem.x], {
                icon: mapItem.iconImage,
                pane: mapItem.pane,
                searchKey: closestTown,
              }).addTo(mapItem.layer)
              ...
              switch (mapItem.teamId) {
                case 1:
                  factionIcon =
                    "<img src='./icons/LogoColonial.png' style='width:24px;height:24px;vertical-align:middle;'/> "
                  break
                case 2:
                  factionIcon =
                    "<img src='./icons/LogoWarden.png' style='width:24px;height:24px;vertical-align:middle;'/> "
                  break
              }

              marker.bindTooltip(
                `${factionIcon}<strong><font color='#d67b52'>${closestTown}</font></strong><br />${mapItem.teamPrefix}${mapItem.description}<br />${mapItem.regionName}`
              )
            } catch (error) {
              console.log(
                `Error - Could not load Map Marker ${mapItem.iconType} in ${mapItem.regionName}.\n` +
                  error
              )
            }
          })
